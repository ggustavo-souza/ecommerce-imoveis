...
@section('content')
    ...
    <div class="container py-4" style="max-width: 1200px;">
        ...
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
                ...
                <div class="col">
                    <div class="card h-100 border-0 shadow-sm card-hover position-relative">
                        ...
                        @auth
                            ...
                            @endphp
                                        <form action="{{ route('favorites.toggle', $product->id) }}" method="POST"
                                            ...
                                            @csrf
                                            <button type="submit"
                                                class="btn {{ $isFavorited ? 'btn-danger' : 'btn-outline-danger' }} btn-sm like-btn">
                        ...
                        <a href="{{ route('products.show', $product->id) }}">
                            @if($product->image)
                                <img src="{{ asset('storage/' . $product->image) }}" class="card-img-top fixed-img"
                                    alt="{{ $product->name }}">
                            @else
                                <img src="https://via.placeholder.com/300x200?text=Sem+Imagem" class="card-img-top fixed-img"
                                    alt="Sem imagem">
                            @endif
                        </a>
                        ...
                        <div
                            class="card-footer d-flex justify-content-between align-items-center bg-transparent border-top-0 pt-0">
                            {{-- Formulário Add Carrinho --}}
                            <form action="{{ route('cart.add', $product->id) }}" method="POST"
                                class="d-flex align-items-center gap-2">
                                @csrf
                                <input type="number" name="quantity" min="1" value="1" max="1"
                                    class="form-control form-control-sm" style="max-width: 70px;">
                                <button type="submit" class="btn bg-orange-500 text-white hover:bg-orange-600 btn-sm">
            ...
            @empty
                <div class="col-12">
                    <div class="alert bg-indigo-400 text-white text-center">
